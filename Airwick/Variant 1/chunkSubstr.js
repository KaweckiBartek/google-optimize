...
const splitString = (str, maxLenght, tag) => {
  let arr = [];
  let string = str;
  ...
  while (string.length) {
    const strr = string.substr(0, maxLenght, tag);
    const part = strr.substr(0, strr.lastIndexOf("}") + 1);
    arr.push(part);
    string = string.substr(part.length, string.length - 1);
  }

  arr.forEach((item) => console.log(`${openTag}${item}${closeTag}`));
};

splitString(string, 480, "style");
